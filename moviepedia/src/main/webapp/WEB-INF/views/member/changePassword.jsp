			...
			<div class="innerFix">
				...
				<div style="margin-left: 50px; margin-top: 70px;">
					...
					<div class="margin--bottom20 padding--h5">
						<p class="margin--bottom5">${userInfo.userid } 회원님의 변경할 비밀번호를 입력해주세요.</p>
...
<!-- 					</form> -->
					<form role="form" class="changePassword padding--h5" method="post" action="/user/changePassword">
							...
							<label value="false"> <!-- label value : false 기본(처음상태, 커서올려논상태) true: 정상입력, 잘못입력 - css위한 클래스만 다름 -->			
								<div class="clearfix">
									<div class="requiredInputParent float--left" >				
										<input id="oldPasswordValidation" class="validationInput" hidden="hidden" value="false">
										<input id="oldPassword" class="requiredInput" autocomplete="off" placeholder="기존 비밀번호" type="password" name="userOldPw" value="">
									</div>
			<!-- 							<div class="btnParent"> -->
							...
							<label value="false"> <!-- label value : false 기본(처음상태, 커서올려논상태) true: 정상입력, 잘못입력 - css위한 클래스만 다름 -->			
								<div class="clearfix">
									<div class="requiredInputParent float--left" >				
										...
										<input id="password" class="requiredInput" autocomplete="off" placeholder="변경할 비밀번호" type="password" name="userNewPw" value="">
									</div>
			<!-- 							<div class="btnParent"> -->
									<div value="false" class="showPwBtnParent float--left">
										<div class="showPwBtn"><i class="fa-regular fa-eye fa-sm"></i></div>
							...
							<label value="false"> <!-- label value : false 기본(처음상태, 커서올려논상태) true: 정상입력, 잘못입력 - css위한 클래스만 다름 -->
								<div class="clearfix">
									<div class="requiredInputParent float--left" >				
										...
										<input id="passwordCheck" class="requiredInput" autocomplete="off" placeholder="변경할 비밀번호 재확인" type="password" name="userpwCheck" value="">
									</div>
									<div value="false" class="showPwBtnParent float--left">
										<div class="showPwBtn"><i class="fa-regular fa-eye fa-sm"></i></div>
										<!-- <i class="fa-regular fa-eye-slash fa-sm"></i> -->
			...
			<!-- 					<p class="warning">비밀번호가 일치하지 않습니다.</p> -->
						...
						<a href="#" style="width:358px;" class="btn  btn-success margin--bottom10">
								<p style="margin:0 auto;" class="font--bold">변경</p></a> 
...
// 		e.stopPropagation();
		...
		if(oldPassword != ''){
			...
			$('#oldPasswordValidation').attr('value', 'true'); // validation true												
			...
			$('#oldPasswordValidation').attr('value', 'false'); // validation false
			$(this).closest('label').children('p').remove(); // p remove
...
// 		e.stopPropagation();
	...
	$(".btn-success").on("click", function(e) {
		...
		if($('#oldPasswordValidation').val() == 'false'){
